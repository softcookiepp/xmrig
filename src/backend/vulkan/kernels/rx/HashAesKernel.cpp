...
void xmrig::HashAesKernel::enqueue(tart::device_ptr queue, size_t threads)
{
...
void xmrig::HashAesKernel::setArgs(tart::buffer_ptr input, tart::buffer_ptr hash, uint32_t hashStrideBytes, uint32_t batch_size)
{
    const uint32_t hashOffsetBytes = 192;

    setArg(0, sizeof(tart::buffer_ptr), &input);
    setArg(1, sizeof(tart::buffer_ptr), &hash);
