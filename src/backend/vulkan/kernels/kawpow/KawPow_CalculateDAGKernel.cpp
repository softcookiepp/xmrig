...
void xmrig::KawPow_CalculateDAGKernel::enqueue(tart::device_ptr queue, size_t threads, size_t workgroup_size)
{
    enqueueNDRange(queue, 1, nullptr, &threads, &workgroup_size);
}


void xmrig::KawPow_CalculateDAGKernel::setArgs(uint32_t start, tart::buffer_ptr g_light, tart::buffer_ptr g_dag, uint32_t dag_words, uint32_t light_words)
{
    setArg(0, sizeof(start), &start);
    setArg(1, sizeof(tart::buffer_ptr), &g_light);
    setArg(2, sizeof(tart::buffer_ptr), &g_dag);
