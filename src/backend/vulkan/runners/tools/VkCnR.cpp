...
#include "backend/vulkan/cl/cn/cryptonight_r_cl.h"
...
private:
    ...
    static std::string getSource(uint64_t offset)
    {
        ...
        for (size_t i = 0; i < VkCnR::kHeightChunkSize; ++i) {
...
{
public:
    inline CnrBaton(const IVkRunner &runner, uint64_t offset) :
        runner(runner),
        offset(offset)
    {}
