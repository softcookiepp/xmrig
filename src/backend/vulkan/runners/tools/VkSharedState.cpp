...
static std::map<uint32_t, VkSharedData> map;


} // namespace xmrig


xmrig::VkSharedData &xmrig::VkSharedState::get(uint32_t index)
{
    return map[index];
}


void xmrig::VkSharedState::release()
{
    for (auto &kv : map) {
        kv.second.release();
    }
...
void xmrig::VkSharedState::start(const std::vector<VkLaunchData> &threads, const Job &job)
{
    assert(map.empty());
