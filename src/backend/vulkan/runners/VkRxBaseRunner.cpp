...
#include "backend/vulkan/kernels/rx/Blake2bInitialHashKernel.h"
#include "backend/vulkan/kernels/rx/Blake2bInitialHashDoubleKernel.h"
#include "backend/vulkan/kernels/rx/Blake2bInitialHashBigKernel.h"
#include "backend/vulkan/kernels/rx/FillAesKernel.h"
#include "backend/vulkan/kernels/rx/FindSharesKernel.h"
...
xmrig::VkRxBaseRunner::VkRxBaseRunner(size_t index, const VkLaunchData &data) : VkBaseRunner(index, data)
{
    ...
    if (data.device.type() == VkDevice::Vega_10 || data.device.type() == VkDevice::Vega_20 || data.device.type() == VkDevice::Raven) {
        m_gcn_version = 14;
    }

    if (data.device.type() == VkDevice::Navi_10 || data.device.type() == VkDevice::Navi_12 || data.device.type() == VkDevice::Navi_14 || data.device.type() == VkDevice::Navi_21) {
        m_gcn_version = 15;
    }
...
xmrig::VkRxBaseRunner::~VkRxBaseRunner()
{
...
void xmrig::VkRxBaseRunner::run(uint32_t nonce, uint32_t nonce_offset, uint32_t *hashOutput)
{
...
void xmrig::VkRxBaseRunner::set(const Job &job, uint8_t *blob)
{
...
size_t xmrig::VkRxBaseRunner::bufferSize() const
{
...
void xmrig::VkRxBaseRunner::build()
{
    VkBaseRunner::build();
...
void xmrig::VkRxBaseRunner::init()
{
    VkBaseRunner::init();
