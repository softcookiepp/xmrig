...
public:
    VkThread() = delete;
    VkThread(uint32_t index, uint32_t intensity, uint32_t worksize, uint32_t stridedIndex, uint32_t memChunk, uint32_t threads, uint32_t unrollFactor) :
        m_threads(threads, -1),
        m_index(index),
        m_memChunk(memChunk),
...
#   ifdef XMRIG_ALGO_RANDOMX
    VkThread(uint32_t index, uint32_t intensity, uint32_t worksize, uint32_t threads, bool gcnAsm, bool datasetHost, uint32_t bfactor) :
...
#   ifdef XMRIG_ALGO_KAWPOW
    VkThread(uint32_t index, uint32_t intensity, uint32_t worksize, uint32_t threads) :
        m_fields(8),
        m_threads(threads, -1),
        m_index(index),
        m_memChunk(0),
...
#   endif
    ...
    VkThread(const rapidjson::Value &value);
    ...
    inline bool operator!=(const VkThread &other) const    { return !isEqual(other); }
    inline bool operator==(const VkThread &other) const    { return isEqual(other); }

    bool isEqual(const VkThread &other) const;
