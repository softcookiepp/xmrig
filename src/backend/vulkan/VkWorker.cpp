...
#   include "backend/vulkan/runners/VkRxVmRunner.h"
#endif
...
std::atomic<bool> VkWorker::ready;


static inline bool isReady()    { return !Nonce::isPaused() && VkWorker::ready; }
...
xmrig::VkWorker::VkWorker(size_t id, const VkLaunchData &data) :
    GpuWorker(id, data.affinity, -1, data.device.index()),
    m_algorithm(data.algorithm),
    m_miner(data.miner),
    m_sharedData(VkSharedState::get(data.device.index()))
{
    switch (m_algorithm.family()) {
    case Algorithm::RANDOM_X:
#       ifdef XMRIG_ALGO_RANDOMX
        if (data.thread.isAsm() && data.device.vendorId() == OCL_VENDOR_AMD) {
            m_runner = new VkRxJitRunner(id, data);
        }
        else {
            m_runner = new VkRxVmRunner(id, data);
        }
#       endif
        break;
...
#       ifdef XMRIG_ALGO_KAWPOW
        m_runner = new VkKawPowRunner(id, data);
#       endif
        break;

    default:
        m_runner = new VkCnRunner(id, data);
        break;
    }
...
xmrig::VkWorker::~VkWorker()
{
    delete m_runner;
}


void xmrig::VkWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool xmrig::VkWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t xmrig::VkWorker::intensity() const
{
...
bool xmrig::VkWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void xmrig::VkWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
